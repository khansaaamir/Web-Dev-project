...
const applyDiscount = require('./middlewares/discount')
const app = express()
...
app.get('/order/preview', applyDiscount, function(req, res){
    let cart = req.session.cart || []
    let total = req.session.discountedTotal || 0
...
app.post('/order/confirm', applyDiscount, function(req, res){
    let cart = req.session.cart || []
    let total = req.session.discountedTotal || 0
...
app.post('/my-orders', function(req, res){
    ...
    .then(function(orders){
        res.render('my-orders-list', { orders: orders })
    })
    .catch(function(err){
